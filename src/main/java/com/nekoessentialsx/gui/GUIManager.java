...
public class GUIManager {
    ...
    public void handleTitlePurchase(Player player, String titleId) {
        ...
        if (balance < TITLE_COST) {
            player.sendMessage("§c呜...你的余额不足的说！需要 " + TITLE_COST + " " + economyManager.getCurrencyName() + "，你现在只有 " + economyManager.formatNumber(balance) + " " + economyManager.getCurrencyName() + " 喵~");
            return;
        }
        ...
        if (economyManager.withdrawPlayer(player, TITLE_COST)) {
            ...
            player.sendMessage("§a已扣除 " + TITLE_COST + " " + economyManager.getCurrencyName() + "，你当前余额: §6" + economyManager.formatNumber(balance - TITLE_COST) + " " + economyManager.getCurrencyName());
    ...
    public void handleConfirmCreateTitle(Player player) {
        ...
        if (!isAdmin) {
            double balance = economyManager.getBalance(player);
            if (balance < cost) {
                player.sendMessage("§c呜...主人的余额不足啦！需要 " + cost + " " + economyManager.getCurrencyName() + ", 你现在只有 " + economyManager.formatNumber(balance) + " " + economyManager.getCurrencyName() + " 喵~");
                return;
            }
        }
